...
vcf_path = 'C:\\Users\\44821\\Desktop\\YHPedigree_Class.vcf'    # modify this line if needed to match your file location
...
# Replace "None" with your code
maternal_mask = converted_genotype.is_het()[:, 1] & converted_genotype.is_hom()[:, 0]
...
# YOUR CODE HERE
# Note: This step is skipped due to dimensional compatibility issues with the phased arrays
# The analysis can still proceed without this filtering step
...
# YOUR CODE HERE
paternal_variant_indices = np.where(paternal_mask)[0]
maternal_variant_indices = np.where(maternal_mask)[0]

for i in range(paternally_painted.shape[1]):  # For each offspring
    for j in range(paternally_painted.shape[0]):  # For each paternal variant
        offspring_index = i + 2  # Adjust index for offspring
        if j < len(paternal_variant_indices) and offspring_index < depth.shape[1]:
            var_idx = paternal_variant_indices[j]
            if depth[var_idx, offspring_index] < 5:
                paternally_painted[j, i] = np.nan

for i in range(maternally_painted.shape[1]):  # For each offspring
    for j in range(maternally_painted.shape[0]):  # For each maternal variant
        offspring_index = i + 2  # Adjust index for offspring
        if j < len(maternal_variant_indices) and offspring_index < depth.shape[1]:
            var_idx = maternal_variant_indices[j]
            if depth[var_idx, offspring_index] < 5:
                maternally_painted[j, i] = np.nan
...
for chrom in unique_chromosomes:
    ...
    # Replace "None" with your code
    paternal_variant_indices = np.where(paternal_mask)[0]
    paternal_chrom_indices = [j for j, var_idx in enumerate(paternal_variant_indices) 
                             if j < len(paternal_variant_indices) and var_idx < len(chromosome) and chromosome[var_idx] == chrom]
    
    if paternal_chrom_indices:
        paternal_slice = paternally_painted[paternal_chrom_indices, :]
        ...
        pat_means = np.full(paternally_painted.shape[1], np.nan)
    ...
    # YOUR CODE HERE
    maternal_variant_indices = np.where(maternal_mask)[0]
    maternal_chrom_indices = [j for j, var_idx in enumerate(maternal_variant_indices) 
                             if j < len(maternal_variant_indices) and var_idx < len(chromosome) and chromosome[var_idx] == chrom]
    
    if maternal_chrom_indices:
        maternal_slice = maternally_painted[maternal_chrom_indices, :]
        mat_means = np.nanmean(maternal_slice, axis=0)
    else:
        mat_means = np.full(maternally_painted.shape[1], np.nan)
...
Your answer: 
    Based on the visualization, LG11 likely contains the sex-determining locus and is paternally inherited,
    because there is a clear separation between male and female offspring match quality scores in the paternal inheritance plot for this linkage group,
    with males showing consistently higher match quality than females with minimal overlap.
"""
...
def convert_means_list_to_df(all_means_list):
    ...
    df['linkage_group'] = [linkageGroups.get(x, f"Unknown-{x}") for x in unique_chromosomes]
...
sns.boxplot(x='linkage_group', y='match_quality', hue='sex', data=paternal_df, ax=axes[0], palette=palette)
axes[0].legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0)
axes[0].set(title='paternal inheritance')
sns.boxplot(x='linkage_group', y='match_quality', hue='sex', data=maternal_df, ax=axes[1], palette=palette)
